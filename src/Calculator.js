import { View, Text, SafeAreaView, FlatList, Pressable, Alert, Switch } from 'react-native'
...
const Calculator = () => {
    ...
    return (
        <SafeAreaView style={{ flex: 1,backgroundColor: theme ? '#0f0f0f' : 'white' }}>
            <View style={{height:800, backgroundColor: theme ? '#0f0f0f' : 'white'}}>
                <Switch
                ...
                />
                <View style={{ height: 300, justifyContent: 'flex-end', alignItems: 'flex-end', paddingBottom: 50, paddingRight: 40 }}>
                    <Text style={{ fontSize: 30,color:theme?'white':'black'}}>{data}</Text>
                    <Text style={{ fontSize: 40,color:theme?'white':'black'}}>{result}</Text>
                </View>

                <View style={{ backgroundColor: theme ? '#0f0f0f' : '#ebedeb', height: 500, borderRadius: 40 }}>
                    <FlatList
                        data={Data}
                        renderItem={render}
                        numColumns={4}
